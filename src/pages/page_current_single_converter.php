<?php
	...
	$getInverter = mysqli_query($connect, "select inverter_serial, inverter_alias from inverters");
						...
						<div  class="pull-right" style="background: #fff; cursor: pointer; padding: 0px 0px; border: 1px solid #ccc">
							<select id="dynamic_data">
								...
								while($row1 = mysqli_fetch_assoc($getInverter)){
									echo "<option value = '" . $row1['inverter_serial'] . "'>" . $row1['inverter_serial'] . " - " . $row1['inverter_alias'] .  "</option>";
								}?>
							</select>
						</div>
					  </div>
