<?php
	...
	$query = mysqli_query($connect,"select date_format(ts,'%H:%i')as ts, dcpower as power from enecsys where id = 0 and date(ts) = curdate() order by ts asc");
	//test query:
	//$query = mysqli_query($connect,"select date_format(ts,'%H:%i:%s')as ts, dcpower from enecsys where id = 0 and date(ts) like '%2016-06-28%' order by ts asc");
